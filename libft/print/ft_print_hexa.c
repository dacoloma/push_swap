...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/03 17:10:40 by dcoloma           #+#    #+#             */
/*   Updated: 2019/09/24 16:53:13 by dcoloma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (size == 1)
		return (ft_u_htoa_base((short)va_arg(ap, int), 16));
	else if (size == 3)
		return (ft_u_hhtoa_base((signed char)va_arg(ap, int), 16));
	else if (size == 2)
		return (ft_u_ltoa_base(va_arg(ap, unsigned long int), 16));
	else if (size == 6)
		return (ft_u_lltoa_base(va_arg(ap, unsigned long long int), 16));
	else
		return (ft_u_itoa_base(va_arg(ap, unsigned int), 16));
}
...
static void		ft_put_prefix(t_option *option, int *i, int len)
{
	...
	if (option->sharp && ft_strcmp(option->data, "0"))
	{
		option->sharp = 0;
		option->buffer[(*i)++] = '0';
		if (option->letter[option->i] == 'x')
			option->buffer[(*i)++] = 'x';
		else if (option->letter[option->i] == 'X')
			option->buffer[(*i)++] = 'X';
		option->width -= 2;
		option->len += 2;
	}
...
static void		ft_get_data(t_option *option, int *i)
{
	...
	if ((!option->precision || option->def_precision)
	...
	if (option->letter[option->i] == 'X')
	{
		...
		while (option->data[j])
		{
			option->data[j] = ft_toupper(option->data[j]);
			j++;
		}
	}
	j = 0;
